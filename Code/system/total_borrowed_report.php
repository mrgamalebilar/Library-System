...
<head>
    ...
    <title>Total Borrowed Report</title>
    <style>
        body {
     ...
     <div style="text-align: center; margin-top: 20px;">
        <a href="borrowed_report.php" class="back-btn">Back to Borrowed Report</a>
    </div>
</head>
<body>
    <?php
    ...
    {
        global $conn;
        $sql = "SELECT COUNT(*) AS total FROM `borrow` WHERE `sex` = '$sex' AND `course_section` = '$course' AND `department` = '$department' AND MONTH(`date_return`) = $month";
    ...
    {
        global $conn;
        $sql = "SELECT COUNT(*) AS total FROM `borrow` WHERE `course_section` = '$course' AND `department` = '$department' AND MONTH(`date_return`) = $month";
        $result = $conn->query($sql);
        if ($result && $result->num_rows > 0) {
            $row = $result->fetch_assoc();
            return $row['total'];
    ...
    $courseQuery = "SELECT DISTINCT `course_section`, `department` FROM `borrow`";
    ...
    if ($courseResult && $courseResult->num_rows > 0) {
        ...
        while ($row = $courseResult->fetch_assoc()) {
            ...
            $typeQuery = "SELECT `type`, COUNT(`type`) AS total_count FROM `borrow` WHERE `course_section` = '$courseSection' AND `department` = '$department' AND MONTH(`datereg`) = $selectedMonth GROUP BY `type`";
    ...
    if (!$dataFound) {
        echo "<div class='no-data'>No Borrowed report found for {$months[$selectedMonth]}.</div>";
    }
