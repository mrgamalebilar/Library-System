...
<head>
    ...
    <style>
     ...
     <div style="text-align: center; margin-top: 20px;">
        <a href="attendance_report.php" class="back-btn">Back to Attendance Report</a>
    </div>
</head>
<body>
    <?php
    ...
    {
        global $conn;
        $sql = "SELECT COUNT(*) AS total FROM `barcode` WHERE `sex` = '$sex' AND `course_section` = '$course' AND `department` = '$department' AND MONTH(`datereg`) = $month";
    ...
    {
        global $conn;
        $sql = "SELECT COUNT(*) AS total FROM `barcode` WHERE `course_section` = '$course' AND `department` = '$department' AND MONTH(`datereg`) = $month";
        $result = $conn->query($sql);
        if ($result && $result->num_rows > 0) {
            $row = $result->fetch_assoc();
            return $row['total'];
    ...
    $courseQuery = "SELECT DISTINCT `course_section`, `department` FROM `barcode`";
    ...
    if ($courseResult && $courseResult->num_rows > 0) {
        ...
        while ($row = $courseResult->fetch_assoc()) {
            ...
            $typeQuery = "SELECT `type`, COUNT(`type`) AS total_count FROM `barcode` WHERE `course_section` = '$courseSection' AND `department` = '$department' AND MONTH(`datereg`) = $selectedMonth GROUP BY `type`";
    ...
    if (!$dataFound) {
        echo "<div class='no-data'>No attendance report found for {$months[$selectedMonth]}.</div>";
    }
