...
<div class="container">
    ...
    <div class="button-group">
        <form method="POST" action="delete_all_return_report.php" class="delete-all-form">
            <button type="submit" class="delete-all-btn">Delete All Returned Books Report</button>
        </form>

        <button onclick="printTable()" class="print-btn">Print</button>
        
        <form method="GET" action="total_returned_report.php" class="calculate-all-form">
            <button type="submit" class="calculate-all-btn">Calculate All Data</button>
        </form>
    </div>
     <!-- Add date selection form -->
     <form method="GET" action="returned_report.php" class="date-filter-form">
        <label for="date">Select Date:</label>
        <input type="date" id="date" name="date">
        <button type="submit" class="filter-btn">Show</button>
    </form>
    <!-- Add refresh button -->
    <form method="GET" action="returned_report.php" class="refresh-form">
...
<table class="returned-books-table">
    ...
    <tbody>
        ...
        if (!empty($dateFilter)) {
            ...
            $query .= " WHERE DATE(`date_return`) = '$dateFilter'";
        }
        ...
        if ($result && $result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>{$row['date_return']}</td>"; // Display the date to be returned
                echo "<td>{$row['returned_date']}</td>"; // Display the actual returned date
                echo "<td>{$row['status']}</td>";
                echo "<td class='action-column'><form method='POST' action='delete_returned.php'>";
            ...
            echo "<tr><td colspan='14' style='color: red; text-align: center;'>No Returned books found.</td></tr>";
        }
