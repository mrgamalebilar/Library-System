...
<head>
    ...
    <title>Total Returned Report</title>
    <style>
        body {
     ...
     <div style="text-align: center; margin-top: 20px;">
        <a href="returned_report.php" class="back-btn">Back to Returned Report</a>
    </div>
</head>
<body>
    <?php
    ...
    {
        global $conn;
        $sql = "SELECT COUNT(*) AS total FROM `returned` WHERE `sex` = '$sex' AND `course_section` = '$course' AND `department` = '$department' AND MONTH(`date_return`) = $month";
    ...
    {
        global $conn;
        $sql = "SELECT COUNT(*) AS total FROM `returned` WHERE `course_section` = '$course' AND `department` = '$department' AND MONTH(`date_return`) = $month";
        $result = $conn->query($sql);
        if ($result && $result->num_rows > 0) {
            $row = $result->fetch_assoc();
            return $row['total'];
    ...
    $courseQuery = "SELECT DISTINCT `course_section`, `department` FROM `returned`";
    ...
    if ($courseResult && $courseResult->num_rows > 0) {
        ...
        while ($row = $courseResult->fetch_assoc()) {
            ...
            $typeQuery = "SELECT `type`, COUNT(`type`) AS total_count FROM `returned` WHERE `course_section` = '$courseSection' AND `department` = '$department' AND MONTH(`date_return`) = $selectedMonth GROUP BY `type`";
    ...
    if (!$dataFound) {
        echo "<div class='no-data'>No Returned report found for {$months[$selectedMonth]}.</div>";
    }
